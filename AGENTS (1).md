...
- **หากยังไม่รู้ `/@<user>/<workspace>/` ของผู้ใช้** — ให้ถามผู้ใช้ บอกให้ user copy `https://<host>/@<user>/<workspace>/apps/code-server/` จากช่อง URL ใน browser มาตอบ
...
### 3.2 Frameworks & Libraries ที่มี/ใช้บ่อย

* **Gradio** — สร้าง web UI สำหรับ ML/AI demo (เวอร์ชัน 5.x+)
* **LangChain / LangGraph** — สร้าง AI agent, workflow, RAG
* **Deep Agents** — สร้าง agent บน LangChain แบบสำเร็จรูป
* **Pydantic + pydantic-settings** — จัดการ config จาก `.env`
* **SQLite3** — database ในตัว ไม่ต้องติดตั้งเพิ่ม

### 3.3 สิ่งที่ไม่มี / ห้ามใช้
